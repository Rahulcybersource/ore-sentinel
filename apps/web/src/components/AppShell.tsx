...
const NAV_ITEMS = [
  ...
  { id: 'satellite', label: 'ISRO Satellite', path: '/satellite', icon: Satellite, roles: ['Geologist', 'Planner'] },
];
...
export const AppShell: React.FC = () => {
  ...
  return (
    <div className="flex h-screen bg-navy-900 text-slate-100 overflow-hidden font-sans">
      ...
      {/* Sidebar Nav */}
      <nav className="w-64 bg-navy-900 border-r border-saffron-500/20 flex flex-col z-20">
        <div className="p-6 border-b border-saffron-500/20">
          <h1 className="text-2xl font-bold text-slate-100 tracking-wide flex items-center gap-2">
            <span className="text-saffron-500">Ore</span>-Sentinel
          </h1>
          <p className="text-[10px] text-slate-400 font-mono mt-1 uppercase tracking-widest flex items-center gap-2">
            ...
            return (
              <NavLink 
                ...
                className={({ isActive }) => `
                  ...
                  ${isActive 
                    ? 'bg-saffron-500/10 text-saffron-400 border-saffron-500/30' 
                    : isEmphasized 
                      ? 'text-slate-200 hover:bg-navy-800 hover:text-saffron-400 border-transparent'
                      : 'text-slate-500 hover:text-slate-300 opacity-60 hover:opacity-100'
                  }
                `}
              >
                <item.icon size={20} className="shrink-0" />
                <span className="font-medium text-sm font-mono uppercase tracking-wider">{item.label}</span>
              </NavLink>
            );
          })}
        </div>
        ...
        <div className="p-4 border-t border-saffron-500/20 bg-navy-950">
          <h3 className="text-[10px] font-bold text-saffron-500/70 mb-2 uppercase tracking-widest">Data Sources & Attribution</h3>
          <ul className="text-[9px] text-slate-500 space-y-1 mb-3 leading-tight font-mono">
            ...
            <li>&bull; Grade thresholds per MOIL published standards.</li>
            <li>&bull; National Mineral Inventory: GSI / IBM UNFC [495.87 Mt].</li>
          </ul>

          {/* Step 13: Data freshness timestamp */}
          {dataTimestamp && (
            <div className="flex items-center gap-1.5 text-[9px] text-slate-400 mb-3 font-mono">
              <Clock size={10} className="shrink-0 text-saffron-500" />
          ...
          <NavLink 
            ...
            className={({ isActive }) => `
              ...
              ${isActive ? 'text-saffron-400 bg-navy-800 border border-saffron-500/30' : 'text-slate-500 hover:text-saffron-400'}
            `}
          >
            Style Guide
          </NavLink>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Top Bar */}
        <header className="h-16 bg-navy-950 border-b border-saffron-500/20 flex items-center justify-between px-8 z-10">
          <div className="flex items-center gap-4">
          </div>

          <div className="flex items-center gap-4">
            <span className="text-[10px] text-saffron-500/70 font-mono uppercase tracking-widest">Active Role:</span>
            <div className="relative">
              <select 
                value={role}
                onChange={(e) => setRole(e.target.value as Role)}
                className="appearance-none bg-navy-900 border border-saffron-500/30 text-saffron-400 font-mono text-xs uppercase tracking-wider py-1.5 pl-4 pr-10 rounded focus:outline-none focus:border-saffron-500 transition-colors"
              >
              ...
              <ChevronDown size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-saffron-500 pointer-events-none" />
            </div>
          </div>
        </header>
