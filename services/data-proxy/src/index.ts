...
const ALLOWED_ORIGINS = config.frontendUrl ? [config.frontendUrl] : ["http://localhost:5173", "http://localhost:3000"];
...
console.log(`COPERNICUS_CLIENT_ID: ${config.copernicusClientId ? 'PRESENT (REDACTED)' : 'MISSING'}`);
console.log(`COPERNICUS_CLIENT_SECRET: ${config.copernicusClientSecret ? 'PRESENT (REDACTED)' : 'MISSING'}`);
console.log(`NASA_EARTHDATA_TOKEN: ${config.nasaEarthdataToken ? 'PRESENT (REDACTED)' : 'MISSING'}`);
console.log(`DATA_GOV_IN_API_KEY: ${config.dataGovInApiKey ? 'PRESENT (REDACTED)' : 'MISSING'}`);
console.log(`ISRO_VEDAS_API_KEY: ${config.isroVedasApiKey ? 'PRESENT (REDACTED)' : 'MISSING'}`);
console.log("=============================");

// STEP 13: OFFLINE/CACHED MODE FLAG
const OFFLINE_MODE = config.offlineMode;
